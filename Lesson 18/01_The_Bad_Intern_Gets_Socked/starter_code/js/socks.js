    ...
    .then(data => {
        ...
        for (product of data) {
            ...
            for (let i = 0; i < product.sizes.length; i++) {
                sizeGroup += `<input type="radio" class="btn-check" name="size_${product.id}" id="size${product.id}_${i}" autocomplete="off" ${checkedString}>
                <label class="btn btn-outline-secondary btn-sm" for="size${product.id}_${i}">${product.sizes[i]}</label>`
        ...
        for (let i = 0; i < colorElementsSet.length; i++) {
            ...
            colorElementsSet[i].addEventListener('click', function(e) {
                ...
                for(let s=0; s < theProductBlock.length; s++){
